...
class Skycrapper {
    constructor(scaleX, scaleY, scaleZ, filling) {
        ...
        let buildingStage1 = new Parts(pencil_parts[14].obj, 10, 0, 0, 0, 1, 0);
...
function loadSkycrapper() {
    ...
    if (Skycrappers.length > 0) {
        console.log("skycrapper loaded !")
        setTimeout(() => {
            showBuidlingMesh();
        }, 2000);
    }
...
//            for (let k = 0; k < Skycrappers[i].body[j].obj.vertices.length - 1; k++) {
//                console.log(k)
...
let meshBuildingLoaded = false;
function showBuidlingMesh() {
    for (let i = 0; i < Skycrappers.length; i++) {
        for (let j = 0; j < Skycrappers[i].body.length; j++) {
            for (let k = 100; k < 440; k++) {
                let newVecor = createVector(Skycrappers[i].body[j].obj.vertices[k].x, Skycrappers[i].body[j].obj.vertices[k].y, Skycrappers[i].body[j].obj.vertices[k].z);
                buildingMesh.push(newVecor);
                //line(Skycrappers[i].body[j].obj.vertices[k].x, Skycrappers[i].body[j].obj.vertices[k].y, Skycrappers[i].body[j].obj.vertices[k].z, Skycrappers[i].body[j].obj.vertices[k + 1].x, Skycrappers[i].body[j].obj.vertices[k + 1].y, Skycrappers[i].body[j].obj.vertices[k + 1].z)
            }
    ...
    setTimeout(() => {
        meshBuildingLoaded = true;
    }, 1000);
}

let Once = true, meshArray = [];
function loadMidMeshBuilding() {
    if (Once) {
        for (let i = 0; i < 10; i++) {
            meshArray.push(i);
        }
    ...
    for (let i = 0; i < meshArray.length; i++) {
        ...
        rotateZ(PI / 2.0);
        scale(50, 50, 3);
        line(buildingMesh[meshArray[i]].x, buildingMesh[meshArray[i]].y, buildingMesh[meshArray[i]].z, buildingMesh[meshArray[i] + 1].x, buildingMesh[meshArray[i] + 1].y, buildingMesh[meshArray[i] + 1].z)
        pop();
    }
}
